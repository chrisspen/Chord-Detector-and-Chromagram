...
Chromagram::Chromagram (int frameSize, int fs)
 :  referenceFrequency (130.81278265), // C3
    bufferSize (8192),
    numHarmonics (2),
    numOctaves (3),
    numBinsToSearch (2)
{
    ...
    noteFrequencies.resize(12*numOctaves);
...
void Chromagram::calculateChromagram()
{
    ...
    for (int n = 0; n < 12; n++)
    {
        ...
        for (int octave = 1; octave <= numOctaves; octave++)
        {
            ...
            int centerBin = round((noteFrequencies[n + (octave-1)*12]) / divisorRatio);
            int minBin = centerBin - (numBinsToSearch);
            int maxBin = centerBin + (numBinsToSearch);
            double maxVal = 0.0;
            ...
            noteSum += maxVal;
            pitches[n + (octave-1)*12] = noteSum;


            noteSum = 0.0;
            for (int harmonic = 1; harmonic <= numHarmonics; harmonic++)
            {
                int centerBin = round((noteFrequencies[n + (octave-1)*12] * octave * harmonic) / divisorRatio);
